  ...
  runDetection(){
    if (this.model != null){
        let predictions = this.model.detect(this.video.nativeElement).then((predictions: any) => {
            ...
            let sum = openhands + closedhands + pointing + pinching;
            if (openhands == 0 && closedhands == 0 && pointing == 0 && pinching == 0)
                this.detectedGesture = "None";

            else if (sum > 1 && (openhands !== 2 && closedhands !== 2 && pointing !== 2 && pinching !== 2)) {
              this.detectedGesture = "";
              if (openhands == 1) {
                this.detectedGesture += "Hand"
              }
              if (closedhands == 1) {
                this.detectedGesture += "Close"
              }
              if (pointing == 1) {
                this.detectedGesture += "Point"
              }
              if (pinching == 1) {
                this.detectedGesture += "Pinch"
              }
            }
